...
wsServer.on('request', function(request) {
  ...
  connection.on('message', function(message) {
    ...
    var socket_index = message.index;
    var cert = message.certificate;
    if(cert == 'certificate'){
      var json = JSON.stringify({ type:'approval', data: 'You are logged in!' });
      ...
      var cert = message.certificate;
      if(cert == certificate){
        var json = JSON.stringify({ type:'approval', data: 'You are logged in!(json issue)' });
        clients[socket_index].sendUTF(json);
      }
    }
  });
